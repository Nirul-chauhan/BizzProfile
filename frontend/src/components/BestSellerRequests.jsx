...
import {
  buyerCreateBestSellerRequest, buyerListBestSellerRequests, getBestSellers,
} from "../api";
import { Package, Star, Send, CheckCircle2, Clock, XCircle, Search } from "lucide-react";

export default function BestSellerRequests() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const data = await buyerListBestSellerRequests();
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try {
      const data = await getBestSellers({ page_size: 20 });
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await buyerCreateBestSellerRequest({
    ...
    <div>
      <div className="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
        <button onClick={() => setActiveTab("requests")} className={`px-4 py-2 text-sm font-bold rounded-lg transition-colors cursor-pointer border-none ${activeTab === "requests" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700 bg-transparent"}`}>
          <Star className="w-4 h-4 inline mr-1.5" />My Requests
        </button>
      ...
      {activeTab === "requests" ? (
        <div>
          {loading ? (
            ...
            <div className="text-center py-12 bg-white border border-gray-200 rounded-xl">
              ...
              <p className="text-gray-500 text-sm mb-3">No best seller requests yet.</p>
              <button onClick={() => setActiveTab("browse")} className="px-4 py-2 bg-orange-500 text-white text-sm font-bold rounded-lg hover:bg-orange-600 cursor-pointer border-none">Browse Products to Request</button>
            </div>
          ) : (
              ...
              {myRequests.map((r) => (
                <div key={r.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {r.message && <p className="text-xs text-gray-500 mt-1 truncate">"{r.message}"</p>}
                  </div>
        ...
        <div>
          {selectedProduct ? (
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
              ...
              <div className="mb-4">
                ...
                <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3} placeholder="Why should this product be a best seller?" className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none resize-none" />
              </div>
              <button onClick={handleSubmit} disabled={submitting} className="w-full py-2.5 bg-orange-500 text-white text-sm font-bold rounded-lg hover:bg-orange-600 disabled:opacity-50 cursor-pointer border-none flex items-center justify-center gap-2">
                <Send className="w-4 h-4" /> {submitting ? "Submitting..." : "Submit Best Seller Request"}
              </button>
            </div>
          ) : null}
          ...
          <div className="flex gap-2 mb-4">
            ...
            <button onClick={handleSearch} disabled={searching} className="px-4 py-2.5 bg-orange-500 text-white text-sm font-bold rounded-xl hover:bg-orange-600 disabled:opacity-50 cursor-pointer border-none flex items-center gap-1">
          ...
          {products.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {products.map((p) => (
                <button key={p.id} onClick={() => setSelectedProduct(p)} className={`text-left p-4 bg-white border-2 rounded-xl hover:shadow-md transition-all cursor-pointer ${selectedProduct?.id === p.id ? "border-orange-500" : "border-gray-100"}`}>
