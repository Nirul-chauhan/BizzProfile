    ...
    <div className="flex gap-3 justify-center" onPaste={handlePaste}>
      {Array.from({ length: 6 }).map((_, i) => (
        <input
          ...
          className="w-13 h-15 text-center text-xl font-bold bg-gray-50 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-gray-900"
        />
      ))}
    </div>
  );
...
export default function CustomerAuth() {
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] flex items-center justify-center px-4 py-12 bg-gradient-to-br from-gray-50 via-white to-emerald-50/30">
      <div className="w-full max-w-lg">
        ...
        <div className="bg-white rounded-3xl p-8 lg:p-10 shadow-xl shadow-gray-200/50 border border-gray-100">
          ...
          <div className="text-center mb-8">
            <div className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg shadow-emerald-500/25">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder="John Doe" value={fullName} onChange={(e) => setFullName(e.target.value)}
                        className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-gray-900 placeholder:text-gray-400 text-base" />
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type={showPassword ? "text" : "password"} placeholder="Create a strong password" value={password} onChange={(e) => setPassword(e.target.value)}
                        className="w-full pl-12 pr-12 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-gray-900 placeholder:text-gray-400 text-base" />
                  ...
                  <div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <input type="tel" placeholder="Enter mobile number" value={mobile}
                        ...
                        className="flex-1 px-5 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-gray-900 placeholder:text-gray-400 text-base" />
                    </div>
                  </div>
                  <button onClick={handleSendOtp} disabled={!canSendOtp}
                    className="w-full py-4 bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 text-white font-bold rounded-2xl hover:from-emerald-600 hover:via-teal-600 hover:to-cyan-600 transition-all shadow-lg shadow-emerald-500/25 disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none cursor-pointer border-none flex items-center justify-center gap-2 text-base">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    {timer > 0 ? (
                      ...
                      <button onClick={handleResend} className="text-sm text-emerald-600 font-bold hover:text-emerald-700 cursor-pointer border-none bg-transparent">Resend OTP</button>
                    )}
                  </div>
                  <button onClick={handleVerifyAndRegister} disabled={otp.length !== 6 || otpVerified || loading}
                    ...
                    {loading ? (
                      ...
                      <>Verify & Register as Customer <ArrowRight className="w-5 h-5" /></>
                    )}
                  </button>
          ...
          {mode === "login" && !forgotMode && (
            <div className="space-y-5">
              <div>
                ...
                <div className="relative">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                  <input type="text" placeholder="you@business.com or +91 1234567890" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)}
                    className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-gray-900 placeholder:text-gray-400 text-base" />
                </div>
              </div>
              <div>
              ...
              <div className="flex justify-end">
                <button onClick={() => { setForgotMode(true); setForgotStep(1); setForgotEmail(loginEmail); setError(""); }}
                  className="text-sm text-emerald-600 font-semibold hover:text-emerald-700 cursor-pointer border-none bg-transparent">
                  Forgot Password?
                </button>
              </div>
              ...
              {forgotStep === 1 && (
                <>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                      <input type="email" placeholder="you@business.com" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)}
                        className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-gray-900 placeholder:text-gray-400 text-base" />
                    </div>
                  </div>
                ...
                <>
                  ...
                  <div className="flex items-center justify-between">
                    {timer > 0 ? (
                      ...
                      <button onClick={handleForgotResendOtp} className="text-sm text-emerald-600 font-bold hover:text-emerald-700 cursor-pointer border-none bg-transparent">Resend OTP</button>
                    )}
                  </div>
