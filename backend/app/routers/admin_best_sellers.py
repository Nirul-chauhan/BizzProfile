"""Admin Best Sellers management router."""
...
router = APIRouter(prefix="/api/admin/best-sellers", tags=["admin-best-sellers"])


class BestSellerToggleRequest(BaseModel):
    is_best_seller: bool


class BestSellerOrderUpdate(BaseModel):
    best_seller_order: int


class BestSellerRequestAction(BaseModel):
...
@router.get("", response_model=list[AdminProductCard])
...
def toggle_best_seller(
    product_id: int,
    data: BestSellerToggleRequest,
    current_user: User = Depends(require_admin),
    db: Session = Depends(get_db_session),
):
    ...
    if not data.is_best_seller:
        product.best_seller_order = 0
    db.commit()
    db.refresh(product)
    return {"id": product.id, "is_best_seller": product.is_best_seller, "best_seller_order": product.best_seller_order}


@router.patch("/{product_id}/order")
def update_best_seller_order(
    product_id: int,
    data: BestSellerOrderUpdate,
    current_user: User = Depends(require_admin),
    db: Session = Depends(get_db_session),
):
    product = db.get(Product, product_id)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    if not product.is_best_seller:
        raise HTTPException(status_code=400, detail="Product is not a best seller")
    product.best_seller_order = data.best_seller_order
    db.commit()
    db.refresh(product)
    return {"id": product.id, "best_seller_order": product.best_seller_order}


# --- Best Seller Requests ---

@router.get("/requests", response_model=list[BestSellerRequestResponse])
def list_best_seller_requests(
    ...
    q = (
        ...
        .options(
            joinedload(BestSellerRequest.buyer),
            joinedload(BestSellerRequest.product),
        )
    )
    if status:
        q = q.where(BestSellerRequest.status == status)
    q = q.order_by(BestSellerRequest.created_at.desc())
...
def review_best_seller_request(
    request_id: int,
    data: BestSellerRequestAction,
    current_user: User = Depends(require_admin),
    db: Session = Depends(get_db_session),
):
    req = db.get(BestSellerRequest, request_id)
    if not req:
        raise HTTPException(status_code=404, detail="Request not found")

    allowed = {BestSellerRequestStatus.APPROVED.value, BestSellerRequestStatus.REJECTED.value}
    ...
    if data.status == BestSellerRequestStatus.APPROVED.value:
